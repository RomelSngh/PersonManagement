...
namespace PersonManagement.Container
{
    public class UserService : IUserService
    {
       ...
       public async Task<APIResponse> ResetPassword(string username, string oldpassword, string newpassword)
        {
            APIResponse response = new APIResponse();
            var _user = await this.context.Users.FirstOrDefaultAsync(item => item.Username == username &&
       ...
       public async Task<APIResponse> ForgetPassword(string username)
        {
            APIResponse response = new APIResponse();
            var _user=await this.context.Users.FirstOrDefaultAsync(item=>item.Username==username && item.Isactive==true);
            if (_user != null)
            {
                string otptext = Generaterandomnumber();
        ...
        {
            var _opt = new PwdManger()
            {
                Username = username,
                Password= password,
                ModifyDate=DateTime.Now
            };
            await this.context.PwdMangers.AddAsync(_opt);
            await this.context.SaveChangesAsync();
        }
        ...
        public async Task<List<UserModel>> Getall()
        {
            ...
            if (_data != null)
            {
                _response = this.mapper.Map<List<User>, List<UserModel>>(_data);
            }
            return _response;
        }
        public async Task<UserModel> Getbycode(string code)
        {
            ...
            var _data = await this.context.Users.FindAsync(code);
            if (_data != null)
            {
                _response = this.mapper.Map<User, UserModel>(_data);
            }
            return _response;
        }
    }
